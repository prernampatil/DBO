function PlotPhasespace(DO,PIDO, BO, DBO, YDO,YDOpi, YBO, YDBO, YExact)
clf
...
if(PIDO)
    plot(YDOpi(1:8:end,1), YDOpi(1:8:end,2), 'p','color', RGB4, LW, 1.5);
    hold on
end
if(BO)
...
h(3) = plot(NaN, NaN, 'p','color', RGB4, LW, 1.5);
h(4) = plot(NaN, NaN, 'x','color', RGB2, LW, 1.5);
h(5) = plot(NaN, NaN, 'sqk', LW, 1.5);
legend(h,'BO','DO', 'PI-DO', 'DBO', 'Analytical');
