% Code to solve the Neumann boundary conditions
...
for NModes = 5:2:9
    ...
    D1 = diag(wp)\D1;
    D2 = -gen_diff_matrix(np, nFE, we, De, nu, bc_left, bc_right);
    D2 = diag(wp)\D2;
    ...
    Phi(1,:) = (gkern(xi,Ut,L,1) -D1(1,2:np+1)*Phi(2:np+1,:))/D1(1,1);
    ...
    for m = 1:nTimeStep-3
        ...
        % Global Error DBO
        Err = Phi - U*S*Y';
        ErrDBO(m) = (sqrt(abs((wp)'*(Err.*Err)*(wr))));
        % Global Error DO
        Err = Phi - UDO*YDO';
        ErrDO(m) = (sqrt(abs((wp)'*(Err.*Err)*(wr))));
        ...
        if(mod(m,200)==0)
            ...
            figure(1)
            plot(x,Phi(:,1:5:25),LW,1.5);
            drawnow
        end
    end
    
    tempstr = sprintf('Neumann_d%d_r%d.mat',Nd,NModes);
